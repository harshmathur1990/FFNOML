...
julia --project=. scripts/validate_gpu_control_timeout.jl
...
The 148/148 unit/integration assertions include HE3D, optional-B MHS, node expansion, observation distribution, rank-local population prediction, threaded synthesis, spatial/spectral PSF, chi-squared, vertical/horizontal regularization, final collection, stable workspace identities, deterministic repeated spectra, stage timings, memory reporting, TOML provenance output, GPU-control diagnostic lifecycle and timeout configuration.
...
The internal status-timeout validator makes rank 0 return later than a 0.3 s
non-root timeout. All three non-root ranks record the timeout, rank 0 records
the failed peer notifications, every rank receives an error, and the same MPI
communicator completes a subsequent launch. Four event logs and four periodic
resource logs are required.
